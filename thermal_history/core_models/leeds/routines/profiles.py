...
def temp_dependent_profiles(model, setup=False):
    ...
    if prm.core_conductivity_params[0] == "external":  # added AR
        k = prm.core_conductivity_params[1](core.conc_l, P, T)
    else:
    ...
    if prm.core_conductivity_params[0] == "AP":
        k = (
            prm.core_conductivity_params[1].astype("float") * core.conc_l[0] ** 2
...
def specific_heat(r, P, cp_params):
    ...
    cp = np.zeros(r.size)
    if cp_params[0] == "external":
        cp = cp_params[1](P)
    elif len(cp_params) == 1:
        cp[:] = cp_params[0]
    else:
...
def thermal_expansivity(r, P, alpha_params):
    ...
    if alpha_params[0] == "external":
        alpha = alpha_params[1](P)
    elif len(alpha_params) == 1:
