...
optional_params = {
    "entrainment_c": ("(Default: 0) Float in the range 0 <= x < 1. Entrainment coefficient, modifies the mass flux out the convecting region by the factor (1-x) to represent entrainment of stable fluid at the base of the stratified layer.", 0),
...
def pure_chemical_method(model):
    ...
    while time_gone < model.dt:
        ...
        try:
            r_s_new = func.retreat_layer(r, T_new, c_new, 0, conc_l, core_adiabat, (resolution, min_res, max_res), alpha_T, alpha_c, density_grad_limit=-lb * alpha_c)
        except:
            breakpoint()
