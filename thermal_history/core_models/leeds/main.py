...
def evolve(model):
    ...
    r, Ta, Ta_grad, g, psi, P, rho, k, cp, alpha = core.profiles["r"], core.profiles["Ta"], core.profiles["dTa_dr"], core.profiles["g"], core.profiles["psi"], core.profiles["P"], core.profiles["rho"], core.profiles["k"], core.profiles["cp"], core.profiles["alpha"]
    ...
    if prm.core_melting_params[0] == "external":
        if any(np.greater(core.profiles["conc_l"], prm.core_melting_params[1].conc_max(1.0e-9 * P))) or any(np.less(core.profiles["conc_l"], prm.core_melting_params[1].conc_min(1.0e-9 * P))):
        ...
        if any(np.greater_equal(core.profiles["conc_l"], eos.xeFeS(1e-9 * P))) and False:
...
def update(model):
    ...
    if prm.iron_snow:
        ...
        core.ri = prof.ic_radius(profiles["r"], prof.adiabat(profiles["r"], core.Tcen, prm.core_adiabat_params), profiles["Tm"])
...
required_params = {
    "T_cmb": "Initial temperature of the CMB. Can specify Tcen (temperature at the center of the core) instead. Float.",
...
optional_params = {
    "include_baro_diffusion": ("(Default: True) Boolean, if True, barodiffusion will be included for both the entropy budget and also chemical gradients in any chemcially stratified layer.", True),
